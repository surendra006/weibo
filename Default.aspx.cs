       ...
       // int pagenum = 0;
        ...
        for (int i = 0; i < 55; i++)
        {
          //  pagenum = i;
            ...
            System.IO.File.WriteAllText(@"C:\Users\User\Desktop\test"+i+".txt", text);           
            int size = ids.Count;
            HtmlDocument document = new HtmlDocument(); 
           // string htmlString = "<html>blabla</html>";
            document.LoadHtml(text);
            HtmlNodeCollection collection = document.DocumentNode.SelectNodes("//dd[@class='content']/p[@node-type='feed_list_content']/em");
            if (!collection.Count.Equals(true))
            {
                foreach (HtmlNode link in collection)
                {
                    string target = link.InnerText;
                    target_full = target_full + target + "\r\n";
                }
            }
            else
                {
            ...
            System.IO.File.WriteAllText(@"C:\Users\User\Desktop\test_agility.txt", target_full);
        }
        //for (int i = 0; i < ids.Count; i++)
        //{
        //    json = json + "" + Sina.API.Statuses.Show(ids[i]).Text.ToString();
        //    System.IO.File.WriteAllText(@"C:\Users\User\Desktop\test.txt", json);
        //}
    }
